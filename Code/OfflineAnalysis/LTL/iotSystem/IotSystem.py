...
def _label_generator(space_dict):
    ...
    for room_name, room in sorted(space_dict.items()):
        temp_dict = {**room["device_dict"],**room["env_state"]}
        for name, value in sorted(temp_dict.items()):
            for ap_name, ap in sorted(value.ap_dict.items()):
                label = label + [str(room["ap_dict"][room_name + '.' + name + '.' + ap_name](value))]
    return label
...
def _description_generator(space_dict):
    description = ''
    for room_name, room in sorted(space_dict.items()):
        for state_name, state_value in sorted(room["state_dict"].items()):
            description = description + '%s=%s, ' % (state_name, str(state_value))
    description = description[:-2]
    return description

def _apply_action(space_dict, action, act_type, env):
    ...
    if act_type == 'device':
        temp[room_name]["action_dict"][action](temp[room_name]["device_dict"][device_name],space_dict)
        return temp
    if act_type == 'state':
        temp[room_name]["action_dict"][action](temp[room_name]["env_state"][device_name],space_dict)
    return temp
...
class IoTSystem(object):
    ...
    def _BFS_item(self, search_pool,state_t, temp_space, room_name ,device_name ,action_name ,device, types):
        if temp_space[room_name]["enable_dict"][room_name + '.' + device_name + '.' + action_name](device):
            temp = _apply_action(temp_space, room_name + '.' + device_name + '.' + action_name, types, self.space_dict)
            new_field = _get_fields(temp)
            print(new_field)
            
            action_description = room_name + '.' + device_name + '.' + action_name
                
            if not self.transition_system.ifStateExists(new_field):
    ...
    def _BFS(self, state_init_t):
        search_pool = [(state_init_t, self.space_dict)]
            ...
            for room_name, room in sorted(temp_space.items()):
                for device_name, device in sorted(room["device_dict"].items()):
                    for action_name, action in device.action_dict.items():
                        self._BFS_item(search_pool,state_t,temp_space,room_name,device_name,action_name,device,'device')
                for state_name, state in sorted(room["env_state"].items()):
                    for action_name, action in state.action_dict.items():
                        self._BFS_item(search_pool,state_t,temp_space,room_name,state_name,action_name,state,'state')
