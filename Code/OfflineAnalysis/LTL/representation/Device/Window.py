...
class Window(BaseType):
    ...
    def action_on(self,env):
        if self.enable_on(self):
            url = "http://47.101.169.122:5002/effect_and_pre/" + self.room + "/Window001/action_on"
            ...
            for effect in effect_and_pre:
                ...
                if effect_and_pre[effect] == '':
                    ...
                    if len(pre_condition_list) == 3:
                        ...
                        temp_two_list = pre_condition_list[2].split('.')
                        temp_three_list = pre_condition_list[4].split('.')
                        temp_four_list = pre_condition_list[8].split('.')
                        value_one = int(pre_condition_list[6])
                        value_two = int(pre_condition_list[10])
                        state_one = env[temp_one_list[0]]["env_state"][temp_one_list[1]]
                        state_two = env[temp_two_list[0]]["env_state"][temp_two_list[1]]
                        device_one = env[temp_three_list[0]]["device_dict"][temp_three_list[1]]
                        device_two = env[temp_four_list[0]]["device_dict"][temp_four_list[1]]
                        if pre_condition_list[1] == '<':
                            if (state_two.ap_high(state_two) == 1 and state_one.ap_high(state_one) == 0 and device_one.ap_on(device_one) == value_one and device_two.ap_on(device_two) == value_two) or (state_two.ap_middle(state_two) == 1 and state_one.ap_low(state_one) == 1 and device_one.ap_on(device_one) == value_one and device_two.ap_on(device_two) == value_two):
                                state = env[temp_list[0]]["env_state"][temp_list[1]]
                                func = getattr(state, temp_list[2], None)
                                func(state,env)
                        elif pre_condition_list[1] == '>':
                            if (state_one.ap_high(state_one) == 1 and state_two.ap_high(state_two) == 0 and device_one.ap_on(device_one) == value_one and device_two.ap_on(device_two) == value_two) or (state_one.ap_middle(state_one) == 1 and state_two.ap_low(state_two) == 1 and device_one.ap_on(device_one) == value_one and device_two.ap_on(device_two) == value_two):
                                state = env[temp_list[0]]["env_state"][temp_list[1]]
                                func = getattr(state, temp_list[2], None)
                                func(state,env)
    ...
    def action_off(self,env):
        if self.enable_off(self):
            url = "http://47.101.169.122:5002/effect_and_pre/" + self.room + "/Window001/action_off"
            effect_and_pre = requests.get(url).json()
            for effect in effect_and_pre:
                temp_list = effect.split('.')
                if effect_and_pre[effect] == '':
