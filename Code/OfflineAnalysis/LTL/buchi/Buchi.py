...
from representation.Environment import getEnvironment, get_dict
...
def _get_field(space_name,space):
    ...
    state_list = [item[1] for item in sorted(temp["state_dict"].items())]
    return state_list
...
class BaseBuchi(object):
    ...
    def get_safty_specification(self,ts,pairs):
        space_dict = deepcopy(getEnvironment()['space_dict'])
        df = pd.DataFrame(columns=('State', 'Action', 'Solve'))
        df_lock = threading.Lock()
        
        solve_dict = dict()
        for edge in self.edge_list:
            if self.state_dict[edge.dst].acc and self.state_dict[edge.src].acc == []:
                for e in self.edge_list:
                    if e.dst == edge.src:
                        src_list = e.ap.split(' & ')
                        dst_list = edge.ap.split(' & ')
                        temp_space_dict = deepcopy(space_dict)
                        for item in src_list:
                            if '!' in item:
                                value = 0
                                temp_item = item[1:].split('.')
                            else:
                                value = 1
                                temp_item = item.split('.')
                            if temp_item[2] == 'on':
                                setattr(temp_space_dict[temp_item[0]]['device_dict'][temp_item[1]],temp_item[2],value)
                            else:
                                setattr(temp_space_dict[temp_item[0]]['env_state'][temp_item[1]],temp_item[2],value)
                        for room in temp_space_dict:
                            temp_room = dict()
                            temp_room["device_dict"] = temp_space_dict[room]["device_dict"]
                            temp_room["env_state"] = temp_space_dict[room]["env_state"]
                            temp = get_dict(room, temp_space_dict[room])
                            temp_room["action_dict"] = temp["action_dict"]
                            temp_room["enable_dict"] = temp["enable_dict"]
                            temp_room["ap_dict"] = temp["ap_dict"]
                            temp_room["state_dict"] = temp["state_dict"]
                            temp_room["ext_action_list"] = temp["ext_action_list"]
                            temp_space_dict[room] = temp_room
                        src_field = _get_fields(temp_space_dict)
                        ...
                        for room in temp_space_dict:
                            ...
                            temp_room["ext_action_list"] = temp["ext_action_list"]
                            temp_space_dict[room] = temp_room
                        dst_field = _get_fields(temp_space_dict)
                        act = ts.findAction(src_field, dst_field)
                        solve_act = edge.ap
                        temp_action = ts.findAction(dst_field, src_field)
                        if edge.ap in solve_dict:
                            temp = solve_dict[edge.ap]
                            if temp_action != None:
                                temp.append(temp_action)
                                solve_dict[edge.ap] = temp
                        else:
                            if temp_action != None:
                                solve_dict[edge.ap] = [temp_action]
                            else:
                                solve_dict[edge.ap] = []
                        with df_lock:
                            df.loc[len(df)] = [e.ap, act, solve_act]
                        break
